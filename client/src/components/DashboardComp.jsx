...
export default function DashboardComp() {
  ...
  const [lastMonthUsers, setLatsMonthUsers] = useState(0);
  const [lastMonthPosts, setLatsMonthPosts] = useState(0);
  const [lastMonthComments, setLatsMonthComments] = useState(0);
    ...
    const fetchUsers = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setLatsMonthUsers(data.lastMonthUsers);
        }
      } catch (error) {
        next(error);
      }
    ...
    const fetchPosts = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setLatsMonthPosts(data.lastMonthPosts);
        }
      } catch (error) {
        next(error);
      }
    ...
    const fetchComments = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setLatsMonthComments(data.lastMonthComments);
        }
      } catch (error) {
        next(error);
      }
    ...
    <div className="p-3 md:mx-auto">
      ...
      <div className="">
        <div className="flex flex-col w-full md:w-auto shadow-md p-2 rounded-md dark:bg-gray-800">
          <div className="flex justify-between font-semibold text-sm p-3">
